 ...
 *
 * Outputs pool capabilities on success. Returns failure if the pool type is not supported as
 * an external memory pool.
 *
 ...
 * ensure HW access. The memory area used for one pool (from the lowest addressed buffer to
 * the end of the highest addressed buffer) must not overlap with the memory area used for
 * any other pool.
 *
